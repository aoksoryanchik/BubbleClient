...
public class ExampleMod implements ModInitializer {
    ...
    private void runAura(MinecraftClient c) {
        ...
        if (auraTarget != null) {
            ...
            // Улучшенный легит-удар: чуть быстрее, но с микроджиттером кулдауна
            boolean canAttack = kaLegit ? (cooldown >= 1.0F && now - lastAttackTime > (50 + rnd.nextInt(70))) : (cooldown >= 1.0F);
    ...
    private void lookAt(PlayerEntity p, Vec3d t, boolean legit) {
        ...
        float speed = legit ? 0.45F : 1.0F; // Увеличил скорость в легите до 0.45 для эффективности
    ...
    public static void saveConfig() {
        try (PrintWriter w = new PrintWriter(new FileWriter(CONFIG_FILE))) {
            w.println(kaRange + ":" + kaWallsRange + ":" + autoRun + ":" + keyKA + ":" + keyTB + ":" + keyFB + ":" + keyAT + ":" + antiVelocity + ":" + tbCrits + ":" + kaLegit);
        ...
        public boolean mouseClicked(double mx, double my, int b) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (mx >= cx - 80 && mx <= cx + 80 && my >= iy && my <= iy + 20) {
                    if (b == 1) { // ПКМ -> Бинд
                        client.setScreen(new BindScreen(this, i)); return true;
                    }
                    if (b == 0) { // ЛКМ -> Включение или Настройки
    ...
    public static class KillAuraSettings extends Screen {
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            btn(ctx, x-170, y+55, 270, 14, "MineBlaze (Legit)", mx, my);
