...
public class ExampleMod implements ModInitializer {
    ...
    private static final String CONFIG_FILE = "bubble_v5_config.txt";
    ...
    private void runAura(MinecraftClient c) {
        ...
        for (PlayerEntity p : c.world.getPlayers()) {
            ...
            // MixerGrief logic needs strict range
            double checkRange = (kaMode == 2) ? 3.1 : kaRange; 
        ...
        if (target != null) {
            ...
            Vec3d pPos = c.player.getEyePos();
            Vec3d diff = tPos.subtract(pPos);
            ...
            if (kaMode == 2) { // MIXER GRIEF (SMOOTH AIM)
                float sens = 0.4f; // Скорость доводки
                float yawDelta = MathHelper.wrapDegrees(yawTo - c.player.getYaw());
                float pitchDelta = MathHelper.wrapDegrees(pitchTo - c.player.getPitch());
                
                c.player.setYaw(c.player.getYaw() + yawDelta * sens);
                c.player.setPitch(c.player.getPitch() + pitchDelta * sens);
            } else { // ARES & BLAZE (SNAP)
                c.player.setYaw(lerpAngle(c.player.getYaw(), yawTo, 1.0f));
                c.player.setPitch(lerpAngle(c.player.getPitch(), pitchTo, 1.0f));
            }
            ...
            if (kaMode == 2) {
                // На MixerGrief бьем только если смотрим на врага (RayTrace)
                if (c.crosshairTarget instanceof EntityHitResult ehr && ehr.getEntity() == target) {
                    canAttack = true;
                } else if (bestDist < 2.5) { // В упор можно бить
                    canAttack = true;
                }
            } else {
                canAttack = true; // На других серверах бьем всегда
            }

            if (canAttack && c.player.getAttackCooldownProgress(0) >= (0.92f + random.nextFloat() * 0.08f)) {
    ...
    private void renderESP(WorldRenderContext context) {
        ...
        VertexConsumerProvider.Immediate cons = c.getBufferBuilders().getEntityVertexConsumers();
        VertexConsumer buf = cons.getBuffer(RenderLayer.getLines());

        for (PlayerEntity p : c.world.getPlayers()) {
            if (p == c.player || !p.isAlive()) continue;
            ms.push();
            double x = MathHelper.lerp(context.tickCounter().getTickDelta(true), p.prevX, p.getX()) - cam.x;
            double y = MathHelper.lerp(context.tickCounter().getTickDelta(true), p.prevY, p.getY()) - cam.y;
            double z = MathHelper.lerp(context.tickCounter().getTickDelta(true), p.prevZ, p.getZ()) - cam.z;
            ms.translate(x, y, z);
            ms.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(-context.camera().getYaw()));
            
            // Цвет меняется в зависимости от режима
            float r = kaMode == 2 ? 0.0f : 1.0f; // Зеленый для Mixer, Красный для других
            float g = kaMode == 2 ? 1.0f : 0.0f;
            
            drawBox(buf, ms.peek().getPositionMatrix(), p.getWidth()/2+0.05f, p.getHeight()+0.05f, r, g, 0.0f, 1f);
            ms.pop();
        }
        cons.draw(RenderLayer.getLines());
    ...
    private void drawBox(VertexConsumer b, Matrix4f m, float w, float h, float r, float g, float bl, float a) {
        ...
        line(b, m, -w, 0, 0, -w, 0, 0, r, g, bl, a); // bottom close
    }
    ...
    private void throwPearl(MinecraftClient c) {
        int s = findItem(c, Items.ENDER_PEARL);
        if (s != -1) runInteract(c, s);
    }
    private void swapElytra(MinecraftClient c) {
        boolean w = c.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.CHEST).isOf(Items.ELYTRA);
        int slot = -1;
        for (int i=0; i<36; i++) {
             ItemStack st = c.player.getInventory().getStack(i);
             if (w ? (st.isOf(Items.NETHERITE_CHESTPLATE) || st.isOf(Items.DIAMOND_CHESTPLATE)) : st.isOf(Items.ELYTRA)) { slot=i; break; }
        }
        if (slot != -1) {
             int t = slot < 9 ? slot + 36 : slot;
             c.interactionManager.clickSlot(c.player.currentScreenHandler.syncId, t, 0, SlotActionType.PICKUP, c.player);
    ...
    private void runTrigger(MinecraftClient c) {
        if (c.crosshairTarget instanceof EntityHitResult e && e.getEntity() instanceof PlayerEntity p) {
             if (p.isAlive() && !friendsList.contains(p.getName().getString().toLowerCase()) && c.player.getAttackCooldownProgress(0) >= 0.9f) {
    ...
    private void checkTotem(MinecraftClient c) {
        if (!c.player.getOffHandStack().isOf(Items.TOTEM_OF_UNDYING)) {
            for (int i=9; i<36; i++) { // Check inventory first
    ...
    public static void saveConfig() {
        try (PrintWriter w = new PrintWriter(new FileWriter(CONFIG_FILE))) {
            w.println(kaRange+":"+kawallsRange+":"+autoRun+":"+keyKA+":"+keyTB+":"+keyAT+":"+friendsRaw+":"+esp+":"+keyESP+":"+fullbright+":"+keyFB+":"+elytraSwap+":"+keyES+":"+fastPearl+":"+keyFP+":"+antiVelocity+":"+kaMode);
    ...
    public static void loadConfig() {
        ...
        try {
            ...
            if (p.length >= 16) {
                ...
                if(p.length > 16) kaMode=Integer.parseInt(p[16]);
            }
        } catch (Exception ignored) {}
    }
    ...
    public static class BubbleMenu extends Screen {
        ...
        @Override public void render(DrawContext ctx, int mx, int my, float delta) {
            ...
            ctx.drawCenteredTextWithShadow(textRenderer, "BUBBLE MENU", cx, cy-80, 0x00CCFF);
            ...
            for (int i=0; i<names.length; i++) {
                ...
                ctx.fill(cx-80, iy, cx+80, iy+20, h ? 0xEE404040 : 0xEE202020);
                ctx.drawCenteredTextWithShadow(textRenderer, names[i], cx, iy+6, states[i] ? 0x00FF00 : 0xFFFFFF);
            }
        }
    ...
    public static class KillAuraSettings extends Screen {
        ...
        @Override protected void init() {
            ...
            fF = new TextFieldWidget(textRenderer, cx+15, cy-75, 100, 14, Text.literal("")); fF.setText(friendsRaw);
            addDrawableChild(rF); addDrawableChild(wF); addDrawableChild(fF);
        }
        @Override public void render(DrawContext ctx, int mx, int my, float delta) {
            renderBackground(ctx, mx, my, delta);
            int cx = width/2, cy = height/2;
            ctx.fill(cx-120, cy-90, cx+130, cy+95, 0xDD101010);
            ctx.drawText(textRenderer, "Range:", cx-115, cy-72, -1, true);
            ctx.drawText(textRenderer, "Walls:", cx-115, cy-52, -1, true);
            
            // PRESETS
            drawBtn(ctx, cx-115, cy-20, "AresMine", mx, my, kaMode==0);
            drawBtn(ctx, cx-30, cy-20, "MineBlaze", mx, my, kaMode==1);
            drawBtn(ctx, cx+55, cy-20, "MixerGrief", mx, my, kaMode==2); // НОВАЯ КНОПКА
            
            drawCheck(ctx, cx-115, cy+10, "AutoRun", autoRun, mx, my);
            drawCheck(ctx, cx+10, cy+10, "AntiVelocity", antiVelocity, mx, my);
            
            // Подсказка режима
            String modeName = kaMode == 0 ? "Ares (Snap)" : kaMode == 1 ? "Blaze (Snap)" : "Mixer (Smooth)";
            ctx.drawCenteredTextWithShadow(textRenderer, "Current Mode: " + modeName, cx, cy+60, 0x00FF00);
        }
        private void drawBtn(DrawContext ctx, int x, int y, String n, int mx, int my, boolean active) {
            boolean h = mx>=x && mx<=x+75 && my>=y && my<=y+15;
            ctx.fill(x, y, x+75, y+15, active ? 0xEE00AA00 : (h ? 0xEE404040 : 0xEE202020));
        ...
        @Override public boolean mouseClicked(double mx, double my, int b) {
            int cx = width/2, cy = height/2;
            if (mx>=cx-115 && mx<=cx-40 && my>=cy-20 && my<=cy-5) setMode(0); // Ares
            if (mx>=cx-30 && mx<=cx+45 && my>=cy-20 && my<=cy-5) setMode(1); // Blaze
            if (mx>=cx+55 && mx<=cx+130 && my>=cy-20 && my<=cy-5) setMode(2); // MixerGrief (New)
        ...
        private void setMode(int m) {
            kaMode = m;
            if (m == 0) { rF.setText("3.8"); wF.setText("3.0"); } // Ares
            if (m == 1) { rF.setText("3.8"); wF.setText("3.0"); } // Blaze
            if (m == 2) { rF.setText("3.0"); wF.setText("0.0"); } // Mixer (Safe)
            saveConfig();
        }
    ...
    public static class BindScreen extends Screen {
        ...
        @Override public void render(DrawContext ctx, int mx, int my, float delta) {
            renderBackground(ctx, mx, my, delta);
            ctx.drawCenteredTextWithShadow(textRenderer, "PRESS KEY", width/2, height/2, 0x00CCFF);
        }
    }
}
