...
public class ExampleMod implements ModInitializer {
    ...
    public static boolean anim1 = false; // Тот самый флаг для Animation 1
        ...
        public boolean mouseClicked(double mx, double my, int b) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (mx >= x - 110 && mx <= x + 110 && my >= cy && my <= cy + 18) {
                    if (i == 0) { kaRange = 3.7; kaWallsRange = 3.0; } 
    ...
    public static class HandSettings extends Screen {
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            int x = width/2, y = height/2;
            ctx.drawCenteredTextWithShadow(textRenderer, "§bVIEWMODEL SETTINGS", x, y - 55, -1);
            
            // Кнопка Animation 1
            boolean h = mx >= x + 10 && mx <= x + 110 && my >= y - 30 && my <= y - 10;
            ctx.fill(x + 10, y - 30, x + 110, y - 10, h ? 0xFF222222 : 0xFF151515);
            ctx.drawCenteredTextWithShadow(textRenderer, "Animation 1: " + (anim1 ? "§aON" : "§cOFF"), x + 60, y - 24, -1);

            ctx.drawTextWithShadow(textRenderer, "X:", x - 95, y - 27, -1);
            ctx.drawTextWithShadow(textRenderer, "Y:", x - 95, y - 5, -1);
            ctx.drawTextWithShadow(textRenderer, "Z:", x - 95, y + 17, -1);
            super.render(ctx, mx, my, d);
        }
        ...
        public boolean mouseClicked(double mx, double my, int b) {
            int x = width/2, y = height/2;
            if (mx >= x + 10 && mx <= x + 110 && my >= y - 30 && my <= y - 10) {
                anim1 = !anim1;
                saveConfig();
                return true;
            }
    ...
    public static void saveConfig() {
        try (PrintWriter w = new PrintWriter(new FileWriter(CONFIG_FILE))) {
            w.println(kaRange + ":" + kaWallsRange + ":" + anim1 + ":" + vmX + ":" + vmY + ":" + vmZ + ":" + autoRun + ":" + antiVelocity + ":" + keyKA + ":" + keyTB + ":" + keyFB + ":" + keyAT + ":" + keyESP + ":" + keyVM);
        } catch (Exception ignored) {}
    }
    ...
    private void loadConfig() {
        ...
        try {
            ...
            if (p.length >= 10) {
                ...
                anim1 = Boolean.parseBoolean(p[2]);
                vmX = Float.parseFloat(p[3]); vmY = Float.parseFloat(p[4]); vmZ = Float.parseFloat(p[5]);
                autoRun = Boolean.parseBoolean(p[6]);
                antiVelocity = Boolean.parseBoolean(p[7]);
            }
        } catch (Exception ignored) {}
    }
