...
public class ExampleMod implements ModInitializer {
    ...
    private void renderWaypoint(WorldRenderContext context) {
        ...
        if (vcp != null) {
            client.textRenderer.draw("§b[!] TARGET", -client.textRenderer.getWidth("[!] TARGET") / 2f, 0, -1, false, ms.peek().getPositionMatrix(), vcp, net.minecraft.client.font.TextRenderer.TextLayerType.SEE_THROUGH, 0, 15728880);
        }
        ms.pop();
    }
    ...
    public static class KillAuraSettings extends Screen {
        ...
        protected void init() {
            ...
            f1.setText(String.format("%.1f", kaRange)); f2.setText(String.format("%.1f", kaWallsRange));
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            drawChk(ctx, "Анти-Отдача", antiVelocity, y + 58, mx, my);
        ...
        public boolean keyPressed(int k, int s, int m) {
            if (k == GLFW.GLFW_KEY_ESCAPE) {
                try { kaRange = Double.parseDouble(f1.getText().replace(",", "."));
                    kaWallsRange = Double.parseDouble(f2.getText().replace(",", "."));
                    shakeIntensity = Float.parseFloat(f3.getText().replace(",", ".")); } catch (Exception ignored) {}
    ...
    public static class WaypointSettings extends Screen {
        ...
        public boolean keyPressed(int k, int s, int m) {
            if (k == GLFW.GLFW_KEY_ESCAPE) {
                try { wpX = Double.parseDouble(f1.getText()); wpY = Double.parseDouble(f2.getText());
                    wpZ = Double.parseDouble(f3.getText()); } catch (Exception ignored) {}
                saveConfig(); client.setScreen(p); return true;
            }
            return f1.keyPressed(k, s, m) || f2.keyPressed(k, s, m) || f3.keyPressed(k, s, m);
        }
    ...
    public static class BindScreen extends Screen {
        ...
        @Override
        public void render(DrawContext ctx, int mx, int my, float d) { ctx.fill(0, 0, width, height, 0xEE000000);
