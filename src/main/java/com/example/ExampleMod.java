...
public class ExampleMod implements ModInitializer {
    ...
    private void swapElytra(MinecraftClient client) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (isWearingElytra) {
                // ФИКС ОШИБКИ: проверка через instanceof и сравнение типа брони напрямую
                if (stack.getItem() instanceof ArmorItem armor) {
                    // В 1.21.4 используем EquipmentSlot напрямую из предмета, если метод маппится иначе
    ...
    private void throwPearl(MinecraftClient client) {
        ...
        if (ps != -1) {
            ...
            // 1.21.4 Fix: Используем 0 как дефолтную последовательность (sequence)
