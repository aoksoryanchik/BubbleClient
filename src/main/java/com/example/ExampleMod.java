...
public class ExampleMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            // Меню на клавишу "0" (верхняя панель)
        ...
        if (client.player.getOffHandStack().getItem() != Items.TOTEM_OF_UNDYING) {
            for (int i = 0; i < 45; i++) {
                if (client.player.getInventory().getStack(i).getItem() == Items.TOTEM_OF_UNDYING) {
                    client.interactionManager.clickSlot(client.player.currentScreenHandler.syncId, i < 9 ? i + 36 : i, 1, SlotActionType.PICKUP, client.player);
                    client.interactionManager.clickSlot(client.player.currentScreenHandler.syncId, 45, 1, SlotActionType.PICKUP, client.player);
                    client.interactionManager.clickSlot(client.player.currentScreenHandler.syncId, i < 9 ? i + 36 : i, 1, SlotActionType.PICKUP, client.player);
                    break;
                }
            }
    ...
    private void runAura(MinecraftClient client) {
        ...
        double minRange = Math.max(0.1, kaRange - 0.1);
        ...
        for (PlayerEntity p : client.world.getPlayers()) {
            ...
            if (d <= kaRange && d >= minRange && d < bestDist) {
    ...
    private void renderWaypoint(MatrixStack ms, net.minecraft.client.render.Camera camera, VertexConsumerProvider vcp) {
        ...
        // Расчет "направления" для проверки, идем ли мы прямо
        Vec3d dirToWp = targetVec.subtract(client.player.getPos()).normalize();
        Vec3d lookVec = client.player.getRotationVec(1.0f).normalize();
        double dot = lookVec.x * dirToWp.x + lookVec.z * dirToWp.z; // Проверка в горизонтальной плоскости
        boolean isLookingAt = dot > 0.985; // ~10 градусов погрешности
        ...
        String text = String.format("§b[!] WAYPOINT (%.1fm)", dist);
        String coords = String.format("§7X: %.0f Y: %.0f Z: %.0f", wpX, wpY, wpZ);
        String arrow = isLookingAt ? "§aV" : "§fV"; // Зеленая если смотрим прямо

        TextRenderer tr = client.textRenderer;
        float w1 = tr.getWidth(text) / 2f;
        float w2 = tr.getWidth(coords) / 2f;
        float w3 = tr.getWidth(arrow) / 2f;

        tr.draw(arrow, -w3, -20, -1, false, ms.peek().getPositionMatrix(), vcp, TextRenderer.TextLayerType.SEE_THROUGH, 0, 15728880);
        tr.draw(text, -w1, -10, -1, false, ms.peek().getPositionMatrix(), vcp, TextRenderer.TextLayerType.SEE_THROUGH, 0, 15728880);
        tr.draw(coords, -w2, 0, -1, false, ms.peek().getPositionMatrix(), vcp, TextRenderer.TextLayerType.SEE_THROUGH, 0, 15728880);
    ...
    public static class BubbleMenu extends Screen {
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                ctx.drawTextWithShadow(textRenderer, n[i] + " §7[" + kN + "]", x + 15, iy + 5, s[i] ? 0xFF00FF80 : 0xFFFF3333);
    ...
    public static class KillAuraSettings extends Screen {
        ...
        public boolean mouseClicked(double mx, double my, int b) {
            ...
            if (mx >= cx + 10 && mx <= cx + 110) {
                ...
                if (my >= y - 20 && my <= y - 2) { kaRange = 3.1; kaWallsRange = 3.0; shakeIntensity = 0.2f; autoRun = true; antiVelocity = false; updateFields(); }
                if (my >= y + 5 && my <= y + 23) { kaRange = 3.02; kaWallsRange = 3.0; shakeIntensity = 0.4f; autoRun = true; antiVelocity = false; updateFields(); }
            }
    ...
    public static class WaypointSettings extends Screen {
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            ctx.drawTextWithShadow(textRenderer, "X:", x - 100, y - 41, -1);
            ctx.drawTextWithShadow(textRenderer, "Y:", x - 100, y - 16, -1);
            ctx.drawTextWithShadow(textRenderer, "Z:", x - 100, y + 9, -1);
            f1.render(ctx, mx, my, d); f2.render(ctx, mx, my, d); f3.render(ctx, mx, my, d);
        }
        @Override
        public boolean keyPressed(int k, int s, int m) {
    ...
    public static class BindScreen extends Screen {
        ...
        public boolean keyPressed(int k, int s, int m) {
            ...
            if (id == 3) keyAT = k; if (id == 4) keyWP = k;
            saveConfig(); client.setScreen(p); return true;
        }
        @Override
        public void render(DrawContext ctx, int mx, int my, float d) {
