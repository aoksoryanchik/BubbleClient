...
public class ExampleMod implements ModInitializer {
    ...
    private void runAura(MinecraftClient c) {
        ...
        for (PlayerEntity p : c.world.getPlayers()) {
            ...
            // Friend List Check
            if (friendsList.contains(p.getEntityName().toLowerCase())) continue;
            ...
            if (kaLegit) {
                ...
                if (Math.abs(MathHelper.wrapDegrees(yaw - c.player.getYaw())) > 85) continue;
            }
            best = d;
            auraTarget = p;
        }
        ...
        if (auraTarget != null) {
            lookAt(c.player, auraTarget.getPos().add(0, auraTarget.getHeight() * 0.6, 0), kaLegit);
            
            float cooldown = c.player.getAttackCooldownProgress(0);
            boolean isFalling = c.player.fallDistance > 0.0F && !c.player.isOnGround() && !c.player.isClimbing() && !c.player.isTouchingWater();
            
            // ЛОГИКА ИДЕАЛЬНОГО КРИТА:
            // Бьем только когда кулдаун > 0.9 И мы либо падаем (крит), либо стоим на месте.
            boolean canAttack = (cooldown >= 0.92F);
            
            // Если мы в прыжке, ждем пока fallDistance станет чуть больше (момент падения вниз)
            if (isFalling && c.player.fallDistance < 0.1F) canAttack = false;
    ...
    private void runTrigger(MinecraftClient c) {
        ...
        if (h != null && h.getType() == HitResult.Type.ENTITY) {
            ...
            if (e instanceof PlayerEntity && e.isAlive() && !friendsList.contains(e.getEntityName().toLowerCase())) {
    ...
    public static void saveConfig() {
        try (PrintWriter w = new PrintWriter(new FileWriter(CONFIG_FILE))) {
            w.println(kaRange + ":" + kaWallsRange + ":" + autoRun + ":" + keyKA + ":" + keyTB + ":" + keyFB + ":" + keyAT + ":" + antiVelocity + ":" + tbCrits + ":" + kaLegit + ":" + friendsRaw);
    ...
    public void loadConfig() {
        ...
        try {
            ...
            if (p.length >= 10) {
                ...
                tbCrits = Boolean.parseBoolean(p[8]); kaLegit = Boolean.parseBoolean(p[9]);
                if (p.length > 10) updateFriends(p[10]);
            }
        } catch (Exception ignored) {}
    }
    ...
    public static class BubbleMenu extends Screen {
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            ctx.fill(cx - 90, cy - 105, cx + 90, cy + 130, -16448251);
            ctx.drawBorder(cx - 90, cy - 105, 180, 235, -16733441);
    ...
    public static class KillAuraSettings extends Screen {
        ...
        @Override protected void init() {
            ...
            rF = new TextFieldWidget(textRenderer, x-60, y-70, 45, 14, Text.of("")); rF.setText(String.valueOf(kaRange));
            wF = new TextFieldWidget(textRenderer, x-60, y-50, 45, 14, Text.of("")); wF.setText(String.valueOf(kaWallsRange));
            // Friend List Input (Меню справа)
            fF = new TextFieldWidget(textRenderer, x + 120, y - 50, 100, 14, Text.of("")); fF.setText(friendsRaw);
        ...
        public void render(DrawContext ctx, int mx, int my, float d) {
            ...
            int x = width/2, y = height/2;
            // Основное окно
            ctx.fill(x-180, y-95, x+110, y+115, -16448251); ctx.drawBorder(x-180, y-95, 290, 210, -16733441);
            // Окно друзей (Справа)
            ctx.fill(x+115, y-95, x+230, y+30, -16448251); ctx.drawBorder(x+115, y-95, 115, 125, -16733441);
            
            ctx.drawCenteredTextWithShadow(textRenderer, "KA SETTINGS", x - 35, y-88, -1);
            ctx.drawCenteredTextWithShadow(textRenderer, "FRIENDS", x + 172, y-88, 0xFF55FF55);
            ctx.drawCenteredTextWithShadow(textRenderer, "(use comma)", x + 172, y-65, 0xFFAAAAAA);
            ...
            drawBtn(ctx, x-170, y-5, 200, 14, "AntiVelocity: ", antiVelocity, mx, my);
            
            ctx.fill(x + 40, y - 25, x + 100, y + 15, kaLegit ? 0xAA00FF00 : 0xAAFF0000);
            ctx.drawCenteredTextWithShadow(textRenderer, "LEGIT", x + 70, y - 10, -1);
            ctx.drawCenteredTextWithShadow(textRenderer, kaLegit ? "ВКЛ" : "ВЫКЛ", x + 70, y, -1);

            ctx.drawCenteredTextWithShadow(textRenderer, "CFG", x - 35, y+45, -1);
            btn(ctx, x-170, y+55, 270, 14, "MineBlaze (Smart Crits)", mx, my);
        ...
        public boolean mouseClicked(double mx, double my, int b) {
            ...
            if (mx >= x-170 && mx <= x+100) {
                ...
                if (my >= y+55 && my <= y+69) { kaRange=3.3; kaWallsRange=0.0; kaLegit=true; rF.setText("3.3"); wF.setText("0.0"); }
