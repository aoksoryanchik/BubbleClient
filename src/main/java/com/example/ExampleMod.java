...
public class ExampleMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            // Открытие меню на 'U' (или другую клавишу, если настроено)
            if (isPressed(h, GLFW.GLFW_KEY_U) && client.currentScreen == null) {
        ...
        HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
            ...
            String arrow = Math.abs(angleDiff) < 10 ? "↑" : (angleDiff > 0 ? "→" : "←");
            String text = String.format("%.0f, %.0f, %.0f | %s %.1fm", wpX, wpY, wpZ, arrow, dist);
    ...
    private void runAura(MinecraftClient client) {
        // Поиск ближайшей цели (невидимки теперь НЕ игнорируются)
    ...
    private void runTrigger(MinecraftClient client) {
        ...
        // Raycast также будет находить невидимок, так как мы не фильтруем их здесь
    ...
    public static void saveConfig() {
        try (PrintWriter w = new PrintWriter(new FileWriter(CONFIG_FILE))) {
            w.println(kaRange + ":" + kaWallsRange + ":" + wpX + ":" + wpY + ":" + wpZ + ":0:0:0:0:" + autoRun + ":" + keyKA + ":" + keyTB + ":" + keyFB + ":" + keyAT + ":" + keyWP + ":0.5:" + antiVelocity + ":" + tbCrits);
    ...
    public static void loadConfig() {
        ...
        try {
            ...
            if (p.length >= 17) {
                ...
                keyKA = Integer.parseInt(p[10]); keyTB = Integer.parseInt(p[11]); keyFB = Integer.parseInt(p[12]);
                keyAT = Integer.parseInt(p[13]); keyWP = Integer.parseInt(p[14]);
                antiVelocity = Boolean.parseBoolean(p[16]);
                tbCrits = Boolean.parseBoolean(p[17]);
            }
        } catch (Exception ignored) {}
    }
    ...
    public static class WaypointSettings extends Screen {
        ...
        protected void init() {
            int x = width / 2 + 20;
            f1 = new TextFieldWidget(client.textRenderer, x, height / 2 - 45, 50, 14, Text.literal(""));
            f2 = new TextFieldWidget(client.textRenderer, x, height / 2 - 25, 50, 14, Text.literal(""));
            f3 = new TextFieldWidget(client.textRenderer, x, height / 2 - 5, 50, 14, Text.literal(""));
